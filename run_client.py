...
async def main():
    ...
    structure = await my_client.call_tool(tool_name="generate_content_structure", topic="How to build a profitable tech startup in 2025?")
    print(structure.content[0].text)

    result = await my_client.call_tool(tool_name="assign_author", topic= structure.content[0].text, authors=[
